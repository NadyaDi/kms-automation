...
class Test:
    ...
    # Test description:
    # create several channel (with different users) and add one user (different one that those how createded channel) and add him as channel member
    # login with the user that was added as member -> go to my channels page and  Filter by 'Channels I am member of'
    # Search for an exsisting channel - The channel you have searched for should be displayed
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 3: Going to add user '" + self.userName2 +"' as member to channel '" + self.channelName1 + "'")
            if self.common.channel.addMembersToChannel(self.channelName1, self.userName2, permission = enums.ChannelMemberPermission.MEMBER) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to add user '" + self.userName2 + "' as member to channel '" + self.channelName1 + "'")
                return
            
            sleep(3)
            writeToLog("INFO","Step 4: Going to logout from " + localSettings.LOCAL_SETTINGS_LOGIN_USERNAME + " user")
            if self.common.login.logOutOfKMS() == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to logout from " + localSettings.LOCAL_SETTINGS_LOGIN_USERNAME + " user")
                return  
            ...
            writeToLog("INFO","Step 6: Going to create new channel")            
            if self.common.channel.createChannel(self.channelName2, self.channelDescription, self.channelTags, enums.ChannelPrivacyType.OPEN, False, True, True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED create new channel: " + self.channelName2)
                return
            ...
            writeToLog("INFO","Step 8: Going to add user '" + self.userName2 +"' as member to channel '" + self.channelName2 + "'")
            if self.common.channel.addMembersToChannel(self.channelName2, self.userName2, permission = enums.ChannelMemberPermission.MEMBER) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED to add user '" + self.userName2 + "' as member to channel '" + self.channelName2 + "'")
                return
            ...
            writeToLog("INFO","Step 12: Going to filter view channel by 'Channels  I am a member of'")
            if self.common.channel.selectViewChannelFilterInMyChannelsPage(self.filterBy) == False:
                ...
                #self.driver.find_elements_by_xpath("//ul[@id='channelGallery']")
            ##################################################################
