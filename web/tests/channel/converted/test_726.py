...
class Test:
    ...
    userPass4 = "Kaltura1!"
    filterByList = "I am subscribed to"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.filterByList = ("I Manage", "I am a member of", "I am subscribed to", "Shared Repositories I am a member of")
            ...
            for i in range(len(self.filterByList)):
                writeToLog("INFO","Step 13: Going to filter view channel by 'Channels '" + self.filterByList[i] + "'")
                if self.common.channel.selectViewChannelFilterInMyChannelsPage(self.filterByList[i]) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 13: FAILED to filter view channels by: " + self.filterByList[i])
                    return 
                    
                    channels = self.driver.find_element_by_xpath("//ul[@id='channelGallery']")
                    if self.channelName2 in channels.text == False:
                        writeToLog("INFO","Step 13: FAILED to find channel '" + self.channelName2 + "' after sort by: Channels " + self.filterByList[i])
