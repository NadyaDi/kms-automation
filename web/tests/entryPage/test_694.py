...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 5: Going to publish entry to category from entry page")
            if self.common.myMedia.publishSingleEntry(self.entryName, [self.categoryName], [], publishFrom = enums.Location.ENTRY_PAGE) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 5: FAILED to publish entry to category from entry page")
                return                 
            
            writeToLog("INFO","Step 6: Going to navigate to category")
            if self.common.category.navigateToCategory(self.categoryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to navigate to category")
                return             
                          
            writeToLog("INFO","Step 7: Going to search entry in category")
            if self.common.category.searchEntryInCategory(self.entryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED to find entry in category")
                return                                                                           
            #########################################################################
