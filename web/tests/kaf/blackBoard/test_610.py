...
#                 return  
#                
#             writeToLog("INFO","Step 3: Going to set one entry as published")  
#             if self.common.myMedia.publishEntriesFromMyMedia(self.entryName2, "", "", [self.galleryName]) == False:
#                 self.status = "Fail"
#                 writeToLog("INFO","Step 3: FAILED to publish entry '" + self.entryName2 + "'")
#                 return 
            ...
            writeToLog("INFO","Step 6: Going to publish entries to moderated gallery")  
            if self.common.kafGeneric.addNewContentToGallery(self.galleryName, uploadEntrieList, isGalleryModerate=True)== False:
                self.status = "Fail"
                writeToLog("INFO","Stepv6: FAILED to publish entries to moderated gallery")
                return 
            ...
            if self.common.blackBoard.logOutOfBlackBoard() == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED to logout from user: " + self.userName1)
                return  
            ...
            if self.common.blackBoard.loginToBlackBoard(localSettings.LOCAL_SETTINGS_LOGIN_USERNAME, localSettings.LOCAL_SETTINGS_LOGIN_PASSWORD) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED to login with " + self.userName1)
                return
               
            writeToLog("INFO","Step 9: Going to reject entry form gallery")  
            if self.common.kafGeneric.handlePendingEntriesIngallery(self.galleryName, self.entryName3, "") == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to reject entry '" + self.entryName3 + "' from gallery '" + self.galleryName + "'")
                return 
            ...
            writeToLog("INFO","Step 11: Going to verify that only entries with " + enums.EntryPrivacyType.PRIVATE.value + " icon display")
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.PRIVATE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 11: FAILED to filter my media entries by '" + enums.EntryPrivacyType.PRIVATE.value + "'")
                    return
            else:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.PRIVATE) == False:
                    self.status = "Fail"
            ...
            writeToLog("INFO","Step 12: Going to verify my media entries by: " + enums.EntryPrivacyType.PRIVATE.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByPrivate) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to verify my media entries  by '" + enums.EntryPrivacyType.PRIVATE.value + "'")
                return 
            
            writeToLog("INFO","Step 13: Going to verify entry privacy label: " + enums.EntryPrivacyType.PRIVATE.value)  
            if self.common.myMedia.verifyEntryPrivacyInMyMedia(self.entryName1, enums.EntryPrivacyType.PRIVATE, forceNavigate=False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 13: FAILED to verify that entry '" + self.entryName1 + "' label is '" + enums.EntryPrivacyType.PRIVATE.value + "'")
                return 
            ...
            sleep(1)
            writeToLog("INFO","Step 14: Going to verify that only entries with " + enums.EntryPrivacyType.PUBLISHED.value + " icon display")
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.PUBLISHED) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 14: FAILED to filter my media entries by '" + enums.EntryPrivacyType.PUBLISHED.value + "'")
                    return
            else:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.PUBLISHED) == False:
                    self.status = "Fail"
            ...
            writeToLog("INFO","Step 15: Going to verify my media entries by: " + enums.EntryPrivacyType.PUBLISHED.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByPublished) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 15: FAILED to verify my media entries  by '" + enums.EntryPrivacyType.PUBLISHED.value + "'")
                return 
            
            writeToLog("INFO","Step 16: Going to verify entry privacy label: " + enums.EntryPrivacyType.PUBLISHED.value)  
            if self.common.myMedia.verifyEntryPrivacyInMyMedia(self.entryName3, enums.EntryPrivacyType.PUBLISHED, forceNavigate=False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 16: FAILED to verify that entry '" + self.entryName3 + "' label is '" + enums.EntryPrivacyType.PUBLISHED.value + "'")
                return
            ...
            sleep(1)
            writeToLog("INFO","Step 17: Going to verify that only entries with " + enums.EntryPrivacyType.REJECTED.value + " icon display")
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.REJECTED) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 17: FAILED to filter my media entries by '" + enums.EntryPrivacyType.REJECTED.value + "'")
                    return
            else:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.REJECTED) == False:
                    self.status = "Fail"
            ...
            writeToLog("INFO","Step 18: Going to verify my media entries by: " + enums.EntryPrivacyType.REJECTED.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByRejected) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 18: FAILED to verify my media entries  by '" + enums.EntryPrivacyType.REJECTED.value + "'")
                return 
            
            writeToLog("INFO","Step 19: Going to verify entry privacy label: " + enums.EntryPrivacyType.REJECTED.value)  
            if self.common.myMedia.verifyEntryPrivacyInMyMedia(self.entryName4, enums.EntryPrivacyType.REJECTED, forceNavigate=False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 19: FAILED to verify that entry '" + self.entryName4 + "' label is '" + enums.EntryPrivacyType.REJECTED.value + "'")
                return
            ...
            sleep(1)
            writeToLog("INFO","Step 20: Going to verify that only entries with " + enums.EntryPrivacyType.PENDING.value + " icon display")
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.PENDING) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 20: FAILED to filter my media entries by '" + enums.EntryPrivacyType.PENDING.value + "'")
                    return
            else:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.PENDING) == False:
                    self.status = "Fail"
            ...
            writeToLog("INFO","Step 21: Going to verify my media entries by: " + enums.EntryPrivacyType.PENDING.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByPending) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 21: FAILED to verify my media entries  by '" + enums.EntryPrivacyType.PENDING.value + "'")
                return 
            
            writeToLog("INFO","Step 22: Going to verify entry privacy label: " + enums.EntryPrivacyType.PENDING.value)  
            if self.common.myMedia.verifyEntryPrivacyInMyMedia(self.entryName5, enums.EntryPrivacyType.PENDING, forceNavigate=False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 22: FAILED to verify that entry '" + self.entryName5 + "' label is '" + enums.EntryPrivacyType.PENDING.value + "'")
                return
            ...
            sleep(1)
            writeToLog("INFO","Step 23: Going to verify that only entries with " + enums.EntryPrivacyType.ALL_STATUSSES.value + " icon display")
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.ALL_STATUSSES) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 23: FAILED to filter my media entries by '" + enums.EntryPrivacyType.ALL_STATUSSES.value + "'")
                    return
            else:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PRIVACY ,enums.EntryPrivacyType.ALL_STATUSSES) == False:
                    self.status = "Fail"
            ...
            writeToLog("INFO","Step 24: Going to verify my media entries by: " + enums.EntryPrivacyType.ALL_STATUSSES.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByAllMedia) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 24: FAILED to verify my media entries  by '" + enums.EntryPrivacyType.ALL_STATUSSES.value + "'")
                return
            
            writeToLog("INFO","Step 25: Going to verify all entries privacy when filter set to: " + enums.EntryPrivacyType.ALL_STATUSSES.value)  
            if self.common.myMedia.verifyEntriesPrivacyInMyMedia(self.filterByAllMedia) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 25: FAILED to verify all entries privacy when filter set to : " + enums.EntryPrivacyType.ALL_STATUSSES.value)
                return
    ...
    def teardown_method(self,method):
        try:
            ...
            self.common.blackBoard.loginToBlackBoard(self.userName1, self.userPass1)
            self.common.blackBoard.switchToBlackboardIframe()
            self.common.myMedia.deleteEntriesFromMyMedia([self.entryName3, self.entryName4])
            self.common.base.switch_to_default_content()
            self.common.blackBoard.logOutOfBlackBoard()
