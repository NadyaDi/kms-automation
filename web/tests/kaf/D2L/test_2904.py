...
class Test:
    ...
    # Test Name : D2L - Publish from my media (single and multiple)
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 3: Going to handle entry in pending tab")
            if self.common.kafGeneric.handlePendingEntriesIngallery(self.galleryName,"", self.imageEntryName, navigate=True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to handle entry in pending tab")
                return
            ...
            writeToLog("INFO","Step 5: Going to verify that entry '" + self.imageEntryName + "' display in gallery")
            if self.common.channel.searchEntryInChannel(self.imageEntryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 5: FAILED verify that entry '" + self.imageEntryName + "' display in gallery")
                return 
              
            writeToLog("INFO","Step 6: Going to publish multiple entries to gallery")
            if self.common.myMedia.publishEntriesFromMyMedia([self.videoEntryName, self.audioEntryName], '', '', [self.galleryName], showAllEntries=True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to publish multiple entries to gallery")
                return  
             
            writeToLog("INFO","Step 7: Going to handle entries in pending tab")
            if self.common.kafGeneric.handlePendingEntriesIngallery(self.galleryName,"", [self.videoEntryName, self.audioEntryName], navigate=True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED to handle entries in pending tab")
                return
            
            writeToLog("INFO","Step 8: Going navigate to gallery page")         
            if self.common.kafGeneric.navigateToGallery(self.galleryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED navigate to gallery page")
                return  
            
            writeToLog("INFO","Step 9: Going to verify that entry '" + self.audioEntryName + "' display in gallery")
            if self.common.channel.searchEntryInChannel(self.audioEntryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED verify that entry '" + self.audioEntryName + "' display in gallery")
                return 
            
            writeToLog("INFO","Step 10: Going to clear search bar in gallery")
            if self.common.myMedia.clearSearch() == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10: FAILED to clear search bar in gallery")
                return 
                
            writeToLog("INFO","Step 11: Going to verify that entry '" + self.videoEntryName + "' display in gallery")
            if self.common.channel.searchEntryInChannel(self.videoEntryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED verify that entry '" + self.videoEntryName + "' display in gallery")
                return 
           
            ##################################################################
            writeToLog("INFO","TEST PASSED: 'D2L - Publish from my media (single and multiple)' was done successfully")
