    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            if self.common.d2l.loginToD2L(self.newUserId, self.newUserPass) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 46: FAILED to login with gallery's owner")
                return
            ...
            writeToLog("INFO","Step 7: Going to handle entries in Pending tab: rejecting entry #1, Approving entry #2, #3")
            if self.common.kafGeneric.handlePendingEntriesIngallery(self.galleryName, self.entryName1, [self.entryName2, self.entryName3], navigate=False) == False:
                self.status = "Fail"
