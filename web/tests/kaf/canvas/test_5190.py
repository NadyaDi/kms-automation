...
class Test:
    ...
    # Test Name : Canvas Embed on BSE from media gallery - v2
    # Test description:
    # Upload entry
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.announcementName = clsTestService.addGuidToString("EmbedAnnouncementFromMediaGalleryV2", self.testNum)
            ...
            if localSettings.LOCAL_SETTINGS_ENV_NAME == 'ProdNewUI':
                if self.common.canvas.createEmbedAnnouncements(self.announcementName, self.entryName, self.galleryName, enums.Location.MEDIA_GALLARY, False, v3=False) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 6: FAILED to create embed announcement from media gallery")
                    return  
            else:
                if self.common.canvas.createEmbedAnnouncements(self.announcementName, self.entryName, self.galleryName, enums.Location.MEDIA_GALLARY, True, v3=False) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 6: FAILED to create embed announcement from media gallery")
                    return  
            ...
            #########################################################################
            writeToLog("INFO","TEST PASSED: 'Embed on BSE (v2) from media gallery' was done successfully")
