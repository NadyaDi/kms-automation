...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            if self.common.category.addMembersToCategory(self.categoryName, self.userName, permission=enums.CategoryMemberPermission.MEMBER) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to add member to category")
                return  
             
            writeToLog("INFO","Step 5: Going to change member permission")
            if self.common.channel.editChannelMemberPermission(self.userName, permission = enums.ChannelMemberPermission.MODERATOR) == False:
                self.status = "Fail"
