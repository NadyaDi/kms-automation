...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 8: Going to filter category entries by: " + enums.Captions.NOT_AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 8: FAILED to filter category entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return
                 
            writeToLog("INFO","Step 9: Going to verify category entries filter by: " + enums.Captions.NOT_AVAILABLE.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to verify category entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return      
            ##################################################################
