...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - no search - Category page - media tab
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 2: Going to filter category entries by: " + enums.Captions.ALL.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 2: FAILED to filter category entries  by '" + enums.Captions.ALL.value + "'")
                    return
                
            writeToLog("INFO","Step 3: Going to verify category entries filter by: " + enums.Captions.ALL.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByAll) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to verify category entries  by '" + enums.Captions.ALL.value + "'")
                return
            ...
            writeToLog("INFO","Step 5: Going to filter category entries by: " + enums.Captions.AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 5: FAILED to filter category entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return
                 
            writeToLog("INFO","Step 6: Going to verify category entries filter by: " + enums.Captions.AVAILABLE.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify category entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            ...
            writeToLog("INFO","Step 10: Going to filter category entries by: " + enums.Captions.NOT_AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 10: FAILED to filter category entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return
                 
            writeToLog("INFO","Step 11: Going to verify category entries filter by: " + enums.Captions.NOT_AVAILABLE.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED to verify category entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return      
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in category while using caption filters with no search term")
