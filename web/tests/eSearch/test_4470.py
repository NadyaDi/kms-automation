...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Duration - no search - Category page - media tab
    # Test description:
    # Verify that category entries are properly displayed in media tab while using no search term and filter them by Duration
    #================================================================================================================================
    testNum = "4470"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.entryName4 = "Filter by Duration - Under onehundredandeighty"
            self.entryName5 = "Filter by Duration - Over onehundredandeighty"
            ...
            writeToLog("INFO", "STEP 2 Going to filter Category page - media tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter Category page - media tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return

            writeToLog("INFO", "STEP 3: Going to verify filter Category page - media tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.channel.verifyEntriesDisplay(self.allDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to verify filter Category page - media tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 5: Going to filter Category page - media tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 5: FAILED to filter Category page - media tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 6: Going to verify filter Category page - media tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.channel.verifyEntriesDisplay(self.tenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to verify filter Category page - media tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 8: Going to filter Category page - media tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 8: FAILED to filter Category page - media tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 9: Going to verify filter Category page - media tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.channel.verifyEntriesDisplay(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter Category page - media tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 11: Going to filter Category page - media tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 11: FAILED to filter Category page - media tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 12: Going to verify filter Category page - media tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.channel.verifyEntriesDisplay(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter Category page - media tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 14: Going to filter Category page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 14: FAILED to filter Category page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 15: Going to verify filter Category page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyEntriesDisplay(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to verify filter Category page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 17: Going to filter Category page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 17: FAILED to filter Category page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")

            writeToLog("INFO", "STEP 18: Going to filter Category page - media tab entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Category page - media tab entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")

            writeToLog("INFO", "STEP 19: Going to verify filter Category page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyEntriesDisplay(self.specialDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to verify filter Category page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
            ...
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Category page - media tab while using filter by duration with no search term")
