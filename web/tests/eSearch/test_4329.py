...
class Test:
    ...
    #================================================================================================================================
    #  @Author: oded berihon
    # Test Name : eSearch - Sort by media type in my media page  
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.entryNameQuiz = "Sort by media type - video - Quiz"
            ...
            writeToLog("INFO","Step 3: Going to filter my media entries by: " + enums.MediaType.IMAGE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.IMAGE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 3: FAILED to filter my media entries  by '" + enums.MediaType.IMAGE.value + "'")
                    return
                
            writeToLog("INFO","Step 4: Going to verify my media entries filter by: " + enums.MediaType.IMAGE.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByImage) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to verify my media entries  by '" + enums.MediaType.IMAGE.value + "'")
                return 
            ...
            sleep(1)
            writeToLog("INFO","Step 5: Going to filter my media entries by: " + enums.MediaType.AUDIO.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.AUDIO) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 5: FAILED to filter my media entries  by '" + enums.MediaType.AUDIO.value + "'")
                    return
                
            writeToLog("INFO","Step 6: Going to verify my media entries filter by: " + enums.MediaType.AUDIO.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByAudio) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify my media entries  by '" + enums.MediaType.AUDIO.value + "'")
                return        
            ...
            sleep(1)
            writeToLog("INFO","Step 7: Going to filter my media entries by: " + enums.MediaType.VIDEO.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.VIDEO) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 7: FAILED to filter my media entries  by '" + enums.MediaType.VIDEO.value + "'")
                    return
                
            writeToLog("INFO","Step 6: Going to verify my media entries filter by: " + enums.MediaType.VIDEO.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByVideo) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify my media entries  by '" + enums.MediaType.VIDEO.value + "'")
                return                     
            ...
            sleep(1)
            writeToLog("INFO","Step 8: Going to filter my media entries by: " + enums.MediaType.QUIZ.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.QUIZ) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 8: FAILED to filter my media entries  by '" + enums.MediaType.QUIZ.value + "'")
                    return
                
            writeToLog("INFO","Step 9: Going to verify my media entries filter by: " + enums.MediaType.QUIZ.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByQuiz) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to verify my media entries  by '" + enums.MediaType.QUIZ.value + "'")
                return                       
