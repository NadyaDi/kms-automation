...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 2: Going to make a search in channel pending tab")
            if self.common.channel.makeSearchInPending(self.entryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 2: FAILED to make a search in channel pending tab")
                return
            
            writeToLog("INFO","Step 3: Going to filter channel pending tab entries by: " + enums.Captions.ALL.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 3: FAILED to filter channel pending tab entries  by '" + enums.Captions.ALL.value + "'")
                    return
               
            writeToLog("INFO","Step 4: Going to verify channel pending tab entries filter by: " + enums.Captions.ALL.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByAll) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to verify channel pending tab entries  by '" + enums.Captions.ALL.value + "'")
                return
            ...
            writeToLog("INFO","Step 6: Going to filter channel pending tab entries by: " + enums.Captions.AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 6: FAILED to filter channel pending tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return
                
            writeToLog("INFO","Step 7: Going to verify channel pending tab entries filter by: " + enums.Captions.AVAILABLE.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED to verify channel pending tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            ...
            writeToLog("INFO","Step 11: Going to filter channel pending tab entries by: " + enums.Captions.NOT_AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 11: FAILED to filter channel pending tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return
                
            writeToLog("INFO","Step 12: Going to verify channel pending tab entries filter by: " + enums.Captions.NOT_AVAILABLE.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to verify channel pending tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return      
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in channel pending tab using a search term")
