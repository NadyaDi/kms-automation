...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Duration - with search - Add new quiz
    # Test description:
    # Verify that Add new quiz entries are properly displayed while using a search term and filter them by Duration
    #================================================================================================================================
    testNum = "4472"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 3: Going to filter Add new quiz entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to filter Add new quiz entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return

            writeToLog("INFO", "STEP 4: Going to verify filter Add new quiz entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.kea.verifyFiltersInAddQuizPage(self.allDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 4: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 6 Going to filter Add new quiz entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to filter Add new quiz entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 7: Going to verify filter global entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.kea.verifyFiltersInAddQuizPage(self.tenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 7: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9: Going to filter Add new quiz entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to filter Add new quiz entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 10: Going to verify filter Add new quiz entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.kea.verifyFiltersInAddQuizPage(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 10: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 12: Going to filter Add new quiz entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to filter Add new quiz entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 13: Going to verify filter Add new quiz entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.kea.verifyFiltersInAddQuizPage(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 13: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 15: Going to filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 16: Going to verify filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.kea.verifyFiltersInAddQuizPage(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 16: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 18: Going to filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 19: Going to filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")
                return

            writeToLog("INFO", "STEP 20: Going to verify filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.kea.verifyFiltersInEditor(self.specialDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 20: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            if self.common.kea.verifyFiltersInEditor(self.betweenDuration) == False:
                ...
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Add new quiz  while using filter by duration with a search term")
