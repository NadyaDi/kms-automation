...
class Test:
    ...
    # Test description:
    # Verify that my media entries are properly displayed while using no search term and filter them by caption
    #================================================================================================================================
    testNum = "4511"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 2: Going to filter My Media page entries by: " + enums.Captions.ALL.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter My Media page entries  by '" + enums.Captions.ALL.value + "'")
                return

            writeToLog("INFO", "STEP 3: Going to verify filter My Media page entries by: " + enums.Captions.ALL.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByAll) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to verify filter My Media page entries  by '" + enums.Captions.ALL.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 5: Going to filter My Media page entries by: " + enums.Captions.AVAILABLE.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 5: FAILED to filter My Media page entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return

            writeToLog("INFO", "STEP 6: Going to verify filter My Media page entries by: " + enums.Captions.AVAILABLE.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to verify filter My Media page entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 8: Going to filter My Media page entries by: " + enums.Captions.NOT_AVAILABLE.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 8: FAILED to filter My Media page entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return

            writeToLog("INFO", "STEP 9: Going to verify filter My Media page entries by: " + enums.Captions.NOT_AVAILABLE.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter My Media page entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
