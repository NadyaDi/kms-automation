...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - with search - Add to channel - SR tab
    # Test description:
    # Verify that "Add To Channe" "SR" entries are properly displayed while using a search term and filter them by captions
    #================================================================================================================================
    testNum = "4457"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 1: Going to navigate to 'Add to channel' - 'SR' tab")
            if self.common.channel.navigateToAddToChannel(self.channelName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to to navigate to 'Add to channel' - 'SR' tab")
                return
            ...
            writeToLog("INFO","Step 5: Going to filter 'Add to channel' - 'SR' tab entries by: " + enums.Captions.ALL.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 5: FAILED to filter 'Add to channel' - 'My Media' tab entries  by '" + enums.Captions.ALL.value + "'")
                    return

            writeToLog("INFO","Step 6: Going to verify 'Add to channel' - 'SR' tab entries filter by: " + enums.Captions.ALL.value)
            if self.common.channel.verifyFiltersInAddToChannel(self.filterByAll, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify 'Add to channel' - 'SR' tab entries  by '" + enums.Captions.ALL.value + "'")
                return

            writeToLog("INFO","Step 7: Going to close down the filters drop down menu")
            if self.common.base.click(self.common.channel.CHANNEL_SR_ADD_TO_CHANNEL_FILTER_BUTTON, 20) == False:
                writeToLog("INFO","Step 7: FAILED to close the filters drop down menu")
                self.status = "Fail"
                return

            writeToLog("INFO","Step 8: Going to filter 'Add to channel' - 'SR' tab entries by: " + enums.Captions.AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 8: FAILED to filter 'Add to channel' - 'SR' tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 9: Going to verify 'Add to channel' - 'SR' tab entries filter by: " + enums.Captions.AVAILABLE.value)
            if self.common.channel.verifyFiltersInAddToChannel(self.filterByAvailable, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to verify 'Add to channel' - 'SR' tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return

            writeToLog("INFO","Step 10: Going to clear all the search filters")
            if self.common.base.click(self.common.channel.CHANNEL_SR_ADD_TO_CHANNEL_CLEAR_ALL_BUTTON, 20) == False:
                writeToLog("INFO","Step 10: FAILED to clear all the search filters")
                self.status = "Fail"
                return

            writeToLog("INFO", "Step 11: Going to wait for the filter changes")
            if self.common.general.waitForLoaderToDisappear() == False:
                writeToLog("INFO", "Step 11: FAILED to clear the filter changes")
                self.status = "Fail"
                return

            writeToLog("INFO","Step 12: Going to close down the filters drop down menu")
            if self.common.base.click(self.common.channel.CHANNEL_SR_ADD_TO_CHANNEL_FILTER_BUTTON, 20) == False:
                writeToLog("INFO","Step 12: FAILED to close the filters drop down menu")
                self.status = "Fail"
                return

            writeToLog("INFO","Step 13: Going to filter 'Add to channel' - 'SR' tab entries by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 13: FAILED to filter 'Add to channel' - 'SR' tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 14: Going to verify 'Add to channel' - 'SR' tab entries filter by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.channel.verifyFiltersInAddToChannel(self.filterByNotAvailable, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 14: FAILED to verify 'Add to channel' - 'SR' tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in 'Add to channel' - 'SR' tab while using caption filters with a search term")
