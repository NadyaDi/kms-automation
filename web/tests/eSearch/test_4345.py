...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 4: Going to select 'search in' option: " + enums.SearchInDropDown.ALL_FIELDS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.ALL_FIELDS) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to select 'search in' option: " + enums.SearchInDropDown.ALL_FIELDS.value)
                return              
            
            writeToLog("INFO","Step 5: Going to verify the correct entries are displayed in results")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.searchInAllFields) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 5: FAILED to displayed correct entries in results")
                return    
            
            writeToLog("INFO","Step 6: Going to select 'search in' option: " + enums.SearchInDropDown.DETAILS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.DETAILS) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to select 'search in' option: " + enums.SearchInDropDown.DETAILS.value)
                return              
            ...
            writeToLog("INFO","Step 8: Going to select 'search in' option: " + enums.SearchInDropDown.CHAPTERS_AND_SLIDES.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.CHAPTERS_AND_SLIDES) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED to select 'search in' option: " + enums.SearchInDropDown.CHAPTERS_AND_SLIDES.value)
                return              
            ...
            writeToLog("INFO","Step 10: Going to select 'search in' option: " + enums.SearchInDropDown.CAPTIONS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.CAPTIONS) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10: FAILED to select 'search in' option: " + enums.SearchInDropDown.CAPTIONS.value)
                return              
            ...
            writeToLog("INFO","Step 12: Going to select 'search in' option: " + enums.SearchInDropDown.POLLS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.POLLS) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to select 'search in' option: " + enums.SearchInDropDown.POLLS.value)
                return              
            ...
            writeToLog("INFO","Step 14: Going to select 'search in' option: " + enums.SearchInDropDown.COMMENTS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.COMMENTS) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 14: FAILED to select 'search in' option: " + enums.SearchInDropDown.COMMENTS.value)
                return              
