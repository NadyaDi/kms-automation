...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Scheduling - Global page
    # Test description:
    # Verify that Global page entries are properly displayed when being filtered by scheduling
    #================================================================================================================================
    testNum = "4485"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 2: Going to filter Global page entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.ALL) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter Global page entries  by '" + enums.Scheduling.ALL.value + "'")
                return
    
            writeToLog("INFO", "STEP 3: Going to verify filter Global page entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.allAvailabilities) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to verify filter Global page entries  by '" + enums.Scheduling.ALL.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 5: Going to filter Global page entries by: " + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.FUTURE_SCHEDULING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 5: FAILED to filter Global page entries  by '" + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
                return
    
            writeToLog("INFO", "STEP 6: Going to verify filter Global page entries by: " + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.futureScheduling) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to verify filter Global page entries  by '" + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 8: Going to filter Global page entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.AVAILABLE_NOW) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 8: FAILED to filter Global page entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return
    
            writeToLog("INFO", "STEP 9: Going to verify filter Global page entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.availableNow) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter Global page entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 11: Going to filter Global page entries by: " + enums.Scheduling.PAST_SCHEDULING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.PAST_SCHEDULING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 11: FAILED to filter Global page entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return
    
            writeToLog("INFO", "STEP 12: Going to verify filter Global page entries by: " + enums.Scheduling.PAST_SCHEDULING.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.pastScheduling) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter Global page entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Global page search while using filter by scheduling")
