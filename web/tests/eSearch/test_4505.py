...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Publish Statuses - no search - My Media
    # Test description:
    # Verify that my media entries are properly displayed while using no search term and filter them by publish status
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 2 Going to filter My Media page entries by: " + enums.EntryPrivacyType.ALL_STATUSSES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PUBLISH_STATUS, enums.EntryPrivacyType.ALL_STATUSSES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter My Media page entries  by '" + enums.EntryPrivacyType.ALL_STATUSSES.value + "'")
                return

            writeToLog("INFO", "STEP 3: Going to verify filter My Media page entries by: " + enums.EntryPrivacyType.ALL_STATUSSES.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.allStatuses) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to verify filter My Media page entries  by '" + enums.EntryPrivacyType.ALL_STATUSSES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 5: Going to filter My Media page entries by: " + enums.EntryPrivacyType.PRIVATE.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PUBLISH_STATUS, enums.EntryPrivacyType.PRIVATE) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 5: FAILED to filter My Media page entries  by '" + enums.EntryPrivacyType.PRIVATE.value + "'")
                return

            writeToLog("INFO", "STEP 6: Going to verify filter My Media page entries by: " + enums.EntryPrivacyType.PRIVATE.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.privateStatus) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to verify filter My Media page entries  by '" + enums.EntryPrivacyType.PRIVATE.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 8: Going to filter My Media page entries by: " + enums.EntryPrivacyType.PUBLISHED.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PUBLISH_STATUS, enums.EntryPrivacyType.PUBLISHED) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 8: FAILED to filter My Media page entries  by '" + enums.EntryPrivacyType.PUBLISHED.value + "'")
                return

            writeToLog("INFO", "STEP 9: Going to verify filter My Media page entries by: " + enums.EntryPrivacyType.PUBLISHED.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.publishedStatus) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter My Media page entries  by '" + enums.EntryPrivacyType.PUBLISHED.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 11: Going to filter My Media page entries by: " + enums.EntryPrivacyType.PENDING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PUBLISH_STATUS, enums.EntryPrivacyType.PENDING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 11: FAILED to filter My Media page entries  by '" + enums.EntryPrivacyType.PENDING.value + "'")
                return

            writeToLog("INFO", "STEP 12: Going to verify filter My Media page entries by: " + enums.EntryPrivacyType.PENDING.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.pendingStatus) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter My Media page entries  by '" + enums.EntryPrivacyType.PENDING.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 14: Going to filter My Media page entries by: " + enums.EntryPrivacyType.REJECTED.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PUBLISH_STATUS, enums.EntryPrivacyType.REJECTED) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 14: FAILED to filter My Media page entries  by '" + enums.EntryPrivacyType.REJECTED.value + "'")
                return

            writeToLog("INFO", "STEP 15: Going to verify filter My Media page entries by: " + enums.EntryPrivacyType.REJECTED.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.rejectedStatus) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to verify filter My Media page entries  by '" + enums.EntryPrivacyType.REJECTED.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 17: Going to filter My Media page entries by: " + enums.EntryPrivacyType.UNLISTED.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.PUBLISH_STATUS, enums.EntryPrivacyType.UNLISTED) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 17: FAILED to filter My Media page entries  by '" + enums.EntryPrivacyType.UNLISTED.value + "'")
                return

            writeToLog("INFO", "STEP 18: Going to verify filter My Media page entries by: " + enums.EntryPrivacyType.UNLISTED.value + "'")
            if self.common.myMedia.verifyFiltersInMyMedia(self.unlistedStatus) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to verify filter My Media page entries  by '" + enums.EntryPrivacyType.UNLISTED.value + "'")
                return

            writeToLog("INFO", "STEP 19: Going to clear the filter search menu")
            if self.common.myMedia.filterClearAllWhenOpened() == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: Failed to clear the search menu")
                return
            ##################################################################
