...
class Test:
    ...
    # Going to add co-publisher permissions to a user for all of the entries
    #================================================================================================================================
    testNum = "31"
    ...
    common = None
    # Test variables
    entryNameVideo      = "Filter by Ownership - thirdtype Video"
    entryNameQuiz       = "Filter by Ownership - thirdtype Quiz"
    entryNameAudio      = "Filter by Ownership - thirdtype Audio"
    entryNameImage      = "Filter by Ownership - thirdtype Image"
    entryNameYoutube    = "Filter by Ownership - thirdtype Youtube"
    entryNameWebCast    = "Filter by Ownership - thirdtype Webcast"
    publishNameQuiz     = "Filter by Ownership - thirdtype Quiz - Quiz"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            for entry in self.entryPermissionList:
                ...
                writeToLog("INFO", "Step " + str(i) + ": Going to add permissions for " + self.publisherUser + " in " + entry + " entry")
                if self.common.editEntryPage.addCollaborator(entry, userId=self.publisherUser, isCoEditor=False, isCoPublisher=True) == False:
                    self.status = "Fail"
                    writeToLog("INFO", "Step" + str(i) + ": FAILED to add permissions for " + self.publisherUser + " in " + entry + " entry")
                    return
                else:
                    i = i + 1
            sleep(10)
