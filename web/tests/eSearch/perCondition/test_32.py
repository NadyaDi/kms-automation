...
class Test:
    ...
    # Going to add co publisher and co editor permissions to a user for all of the entries
    #================================================================================================================================
    testNum = "32"
    ...
    common = None
    # Test variables
    entryNameVideo      = "Filter by Ownership - fourthtype Video"
    entryNameQuiz       = "Filter by Ownership - fourthtype Quiz"
    entryNameAudio      = "Filter by Ownership - fourthtype Audio"
    entryNameImage      = "Filter by Ownership - fourthtype Image"
    entryNameYoutube    = "Filter by Ownership - fourthtype Youtube"
    entryNameWebCast    = "Filter by Ownership - fourthtype Webcast"
    publishNameQuiz     = "Filter by Ownership - fourthtype Quiz - Quiz"
    ...
    publisherAndEditorUser = "unmodForEsearch"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            for entry in self.entryPermissionList:
                ...
                writeToLog("INFO", "Step " + str(i) + ": Going to add permissions for " + self.publisherAndEditorUser + " in " + entry + " entry")
                if self.common.editEntryPage.addCollaborator(entry, userId=self.publisherAndEditorUser, isCoEditor=True, isCoPublisher=True) == False:
                    self.status = "Fail"
                    writeToLog("INFO", "Step" + str(i) + ": FAILED to add permissions for " + self.publisherAndEditorUser + " in " + entry + " entry")
                    return
                else:
                    i = i + 1
            sleep(10)
