...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Duration - no search - Add new quiz
    # Test description:
    # Verify that Add new quiz entries are properly displayed while using no search term and filter them by Duration
    #================================================================================================================================
    testNum = "4473"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 2 Going to filter channel Add new quiz entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter Add new quiz entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return

            writeToLog("INFO", "STEP 3 Going to verify filter Add new quiz entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.kea.verifyFiltersInEditor(self.allDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 5 Going to filter Add new quiz entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 5: FAILED to filter Add new quiz entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 6 Going to verify filter Add new quiz entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.kea.verifyFiltersInEditor(self.tenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 8 Going to filter Add new quiz entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 8: FAILED to filter Add new quiz entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 9 Going to verify filter Add new quiz entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.kea.verifyFiltersInEditor(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 11 Going to filter Add new quiz entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 11: FAILED to filter Add new quiz entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 12 Going to verify filter Add new quiz entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.kea.verifyFiltersInEditor(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 14 Going to filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 14: FAILED to filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 15 Going to verify filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.kea.verifyFiltersInEditor(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to verify filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 17: Going to filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 17: FAILED to filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 18: Going to filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Add new quiz entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")
                return

            writeToLog("INFO", "STEP 19: Going to verify filter Add new quiz entries by: " + enums.Duration.CUSTOM.value + "'")
