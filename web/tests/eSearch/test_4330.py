...
class Test:
    ...
    #================================================================================================================================
    #  @Author: oded berihon
    # Test Name : eSearch - Sort by media type in cahnnel page  
    # Test description:
...
#             New UI only !! this parameter will be clicked after every filter search so each filter will only have only the chosen type 
            ...
            writeToLog("INFO","Step 8: Going to verify my media entries filter by: " + enums.MediaType.VIDEO.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByVideo, verifyIn=enums.Location.CHANNEL_PAGE) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED to verify my media entries  by '" + enums.MediaType.VIDEO.value + "'")
                return                     
            ...
            sleep(1)
            writeToLog("INFO","Step 9: Going to filter my media entries by: " + enums.MediaType.QUIZ.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.QUIZ) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 9: FAILED to filter my media entries  by '" + enums.MediaType.QUIZ.value + "'")
                    return
                 
            writeToLog("INFO","Step 10: Going to verify my media entries filter by: " + enums.MediaType.QUIZ.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByQuiz, verifyIn=enums.Location.CHANNEL_PAGE ) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10: FAILED to verify my media entries  by '" + enums.MediaType.QUIZ.value + "'")
                return                       
