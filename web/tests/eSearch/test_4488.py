...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Scheduling - with search - Category page - media tab
    # Test description:
    # Verify that Category page - media tab entries are properly displayed while being filtered by scheduling with a search term
    #================================================================================================================================
    testNum = "4488"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 3: Going to filter Category page - media tab entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.ALL) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to filter Category page - media tab entries  by '" + enums.Scheduling.ALL.value + "'")
                return

            writeToLog("INFO", "STEP 4: Going to verify filter Category page - media tab entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.allAvailabilities) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 4: FAILED to verify filter Category page - media tab entries  by '" + enums.Scheduling.ALL.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 6: Going to filter Category page - media tab entries by: " + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.FUTURE_SCHEDULING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to filter Category page - media tab entries  by '" + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
                return

            writeToLog("INFO", "STEP 7: Going to verify filter Category page - media tab entries by: " + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.futureScheduling) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 7: FAILED to verify filter Category page - media tab entries  by '" + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9: Going to Category page - media tab global entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.AVAILABLE_NOW) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to filter Category page - media tab entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return

            writeToLog("INFO", "STEP 10: Going to verify filter Category page - media tab entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.availableNow) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 10: FAILED to verify filter Category page - media tab entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 12: Going to filter Category page - media tab entries by: " + enums.Scheduling.PAST_SCHEDULING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.PAST_SCHEDULING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to filter Category page - media tab entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return

            writeToLog("INFO", "STEP 13: Going to verify filter Category page - media tab entries by: " + enums.Scheduling.PAST_SCHEDULING.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.pastScheduling) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 13: FAILED to verify filter Category page - media tab entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Category page - media tab search while using filter by scheduling")
