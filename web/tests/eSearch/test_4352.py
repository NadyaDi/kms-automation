...
class Test:
    ...
    # Test Name : eSearch - Search In - channel - pending tab
    # Test description:
    # Upload entries with different cue points fields
    # Go pending page, media tab, make a search, and filter entries by 'Search in' drop down:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 1: Going to navigate to channel page - pending tab")
            if self.common.channel.navigateToPendingaTab(self.channel) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to go to navigate channel page")
                return      
            ...
            writeToLog("INFO","Step 3: Going to make a search in channel- pending tab")
            if self.common.channel.makeSearchInPending(self.searchInchannel) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to make a search in channel- pending tab")
                return          
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.searchInQuiz) == False:
                self.status = "Fail"
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.searchInAllFields) == False:
                self.status = "Fail"
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.searchInDeatails) == False:
                self.status = "Fail"
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.searchInChaptersAndSlides) == False:
                self.status = "Fail"
            ...
            writeToLog("INFO","Step 17: Going to select 'search in' option: " + enums.SearchInDropDown.CAPTIONS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.CAPTIONS, location=enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 17: FAILED to select 'search in' option: " + enums.SearchInDropDown.CAPTIONS.value)
                return              
            
            writeToLog("INFO","Step 18: Going to verify the correct entries are displayed in results")
            if self.common.channel.verifyFiltersInPendingTab(self.SearchIncaptions) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 18: FAILED to displayed correct entries in results")
                return      
            
            writeToLog("INFO","Step 19: Going to verify fields display for search in" + enums.SearchInDropDown.CAPTIONS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForCaptions) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 19: FAILED to displayed correct fields for " + enums.SearchInDropDown.CAPTIONS.value)
                return            

            writeToLog("INFO","Step 20: Going to select 'search in' option: " + enums.SearchInDropDown.POLLS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.POLLS, location=enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 20: FAILED to select 'search in' option: " + enums.SearchInDropDown.POLLS.value)
                return              
            
            writeToLog("INFO","Step 21: Going to verify the correct entries are displayed in results")
            if self.common.channel.verifyFiltersInPendingTab(self.searchInPolls) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 21: FAILED to displayed correct entries in results")
                return      
            
            writeToLog("INFO","Step 22: Going to verify fields display for search in" + enums.SearchInDropDown.POLLS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForPolls) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 22: FAILED to displayed correct fields for " + enums.SearchInDropDown.POLLS.value)
                return                   
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.searchInComments) == False:
                self.status = "Fail"
            ...
            if self.common.myMedia.verifySearchInDropDownState(False) == False:
                ...
                return                          
            ##################################################################
            writeToLog("INFO","TEST PASSED: Search in channel - pending tab done successfully")
