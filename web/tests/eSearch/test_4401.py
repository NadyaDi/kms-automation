...
class Test:
    ...
    # Test Name : eSearch - Sort by - Add to gallery - My media tab - when no search is made
    # Test description:
    # Upload entries with different sort by values
    # Go to Add to gallery page, my media tab and make the next sorts:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            if self.common.channel.verifySortInAddToChannel(enums.SortBy.UPDATE_DESC, self.sortEntriesByUpdateDateDescending) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED to sort entries by 'Update - Descending' - when search is made")
                return  
            ...
            if self.common.channel.verifySortInAddToChannel(enums.SortBy.UPDATE_ASC, self.sortEntriesByUpdateDateAscending) == False:
                ...
                return                                                         
            ##################################################################
            writeToLog("INFO","TEST PASSED: Sort by in 'Add to gallery - My Media tab' - when no search is made was done successfully")
