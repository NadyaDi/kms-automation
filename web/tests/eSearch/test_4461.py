...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - with search - Add to Gallery - My media tab
    # Test description:
    # Verify that "Add To Gallery" "My Media Tab" entries are properly displayed while using a search term and filter them by captions
    #================================================================================================================================
    testNum = "4461"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 3: Going to make a search in 'Add to Gallery' - 'My Media' tab")
            if self.common.channel.searchInAddToChannel(self.search) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to make a search in 'Add to Gallery' - 'My Media' tab")
                return
            ...
            writeToLog("INFO","Step 5: Going to filter 'Add to Gallery' - 'My Media' tab entries by: " + enums.Captions.ALL.value)
            ...
            writeToLog("INFO","Step 6: Going to verify 'Add to Gallery' - 'My Media' tab entries filter by: " + enums.Captions.ALL.value)
            if self.common.channel.verifyFiltersInAddToChannel(self.filterByAll, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify 'Add to Gallery' - 'My Media' tab entries  by '" + enums.Captions.ALL.value + "'")
                return
            ...
            writeToLog("INFO","Step 8: Going to filter 'Add to Gallery' - 'My Media' tab entries by: " + enums.Captions.AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 8: FAILED to filter 'Add to Gallery' - 'My Media' tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 9: Going to verify 'Add to Gallery' - 'My Media' tab entries filter by: " + enums.Captions.AVAILABLE.value)
            if self.common.channel.verifyFiltersInAddToChannel(self.filterByAvailable, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to verify 'Add to Gallery' - 'My Media' tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            ...
            writeToLog("INFO","Step 11: Going to filter 'Add to Gallery' - 'My Media' tab tab entries by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 11: FAILED to filter 'Add to Gallery' - 'My Media' tab tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 12: Going to verify 'Add to Gallery' - 'My Media' tab entries filter by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.channel.verifyFiltersInAddToChannel(self.filterByNotAvailable, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to verify 'Add to Gallery' - 'My Media' tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in 'Add to Gallery' - 'My Media' tab while using caption filters with a search term")
