...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - with search - Add new quiz
    # Test description:
    # Verify that add new quiz entries are properly displayed while using a search term and filter them by captions
    #================================================================================================================================
    testNum = "4451"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 3: Going to filter add quiz page entries by: " + enums.Captions.ALL.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 3: FAILED to filter add quiz page entries  by '" + enums.Captions.ALL.value + "'")
                    return

            writeToLog("INFO","Step 4: Going to verify add quiz page entries filter by: " + enums.Captions.ALL.value)
            if self.common.kea.verifyFiltersInAddQuizPage(self.filterByAll) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to verify add quiz page entries  by '" + enums.Captions.ALL.value + "'")
                return
            ...
            writeToLog("INFO","Step 6: Going to filter add quiz page entries by: " + enums.Captions.AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 6: FAILED to filter add quiz page entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 7: Going to verify add quiz page entries filter by: " + enums.Captions.AVAILABLE.value)
            if self.common.kea.verifyFiltersInAddQuizPage(self.filterByAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED to verify add quiz page entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            ...
            writeToLog("INFO","Step 11: Going to filter add quiz page entries by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 11: FAILED to filter add quiz page entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 12: Going to verify add quiz page entries filter by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.kea.verifyFiltersInAddQuizPage(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to verify add quiz page entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in add new quiz page while using caption filters with a search term")
