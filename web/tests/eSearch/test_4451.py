...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 9: Going to filter add quiz page entries by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 9: FAILED to filter add quiz page entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 10: Going to verify add quiz page entries filter by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.kea.verifyFiltersInAddQuizPage(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10: FAILED to verify add quiz page entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
