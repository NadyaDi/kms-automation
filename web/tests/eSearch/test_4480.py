...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Duration - with search - Add to Gallery - My media tab
    # Test description:
    # Verify that "Add To Gallery" "My Media Tab" entries are properly displayed while using a search term and filter them by duration
    #================================================================================================================================
    testNum = "4480"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 3 Going to filter Add To Gallery My Media Tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to filter Add To Gallery My Media Tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return

            writeToLog("INFO", "STEP 4 Going to verify filter Add To Gallery My Media Tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.allDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 4: FAILED to verify filter Add To Gallery My Media Tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 6 Going to filter Add To Gallery My Media Tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to filter Add To Gallery My Media Tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 7 Going to verify filter Add To Gallery My Media Tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.tenDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 7: FAILED to verify filter Add To Gallery My Media Tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9 Going to filter Add To Gallery My Media Tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to filter Add To Gallery My Media Tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return


            writeToLog("INFO", "STEP 10: Going to verify filter Add To Gallery My Media Tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.thirtyDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 10: FAILED to verify filter Add To Gallery My Media Tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 12 Going to filter Add To Gallery My Media Tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to filter Add To Gallery My Media Tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return


            writeToLog("INFO", "STEP 13 Going to verify filter Add To Gallery My Media Tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.sixtyDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 13: FAILED to verify filter Add To Gallery My Media Tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 15 Going to filter Add To Gallery My Media Tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to filter Add To Gallery My Media Tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 16 Going to verify filter Add To Gallery My Media Tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.customDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 16: FAILED to verify filter Add To Gallery My Media Tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 18 Going to filter Add to Gallery - My media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Add to Gallery - My media tab entries  by '" + enums.Duration.CUSTOM.value + "'")

            writeToLog("INFO", "STEP 19 Going to filter Add to Gallery - My media tab entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to filter Add to Gallery - My media tab entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")

            writeToLog("INFO", "STEP 20 Going to verify filter Add to Gallery - My media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.specialDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 20: FAILED to verify filter Add to Gallery - My media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
            ...
            if self.common.channel.verifyFiltersInAddToChannel(self.betweenDuration, enums.Location.ADD_TO_CHANNEL_MY_MEDIA) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 26: FAILED to verify filter Global page entries  by '" + enums.Duration.CUSTOM.value + "'")
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Add to Gallery My Media Tab while using filter by duration with a search term")
