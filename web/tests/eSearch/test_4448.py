...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - no search - Channel page - pending tab
    # Test description:
    # Verify that channel pending tab entries are properly displayed while using no search term and filter them by captions
    #================================================================================================================================
    testNum = "4448"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 1: Going to navigate to the channel pending tab")
            if self.common.channel.navigateToPendingaTab(self.channelName, enums.Location.CHANNEL_PAGE) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to navigate to the channel pending tab")
                return
            
            writeToLog("INFO","Step 2: Going to filter channel pending tab entries by: " + enums.Captions.ALL.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 2: FAILED to filter channel pending tab entries  by '" + enums.Captions.ALL.value + "'")
                    return
               
            writeToLog("INFO","Step 3: Going to verify channel pending tab entries filter by: " + enums.Captions.ALL.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByAll) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to verify channel pending tab entries  by '" + enums.Captions.ALL.value + "'")
                return
            
            writeToLog("INFO","Step 4: Going to close down the filters drop down menu") 
            if self.common.base.click(self.common.channel.MODERATION_CHANNELS_FILTER_BUTTON, 20) == False:
                writeToLog("INFO","Step 4: FAILED to close the filters drop down menu")
                self.status = "Fail"
                return False       
                
            writeToLog("INFO","Step 5: Going to filter channel pending tab entries by: " + enums.Captions.AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 5: FAILED to filter channel pending tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return
                
            writeToLog("INFO","Step 6: Going to verify channel pending tab entries filter by: " + enums.Captions.AVAILABLE.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify channel pending tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            
            writeToLog("INFO","Step 7: Going to clear all the search filters") 
            if self.common.base.click(self.common.channel.MODERATION_CHANNELS_FILTER_CLEAR_ALL_BUTTON, 20) == False:
            ...
            writeToLog("INFO","Step 10: Going to filter channel pending tab entries by: " + enums.Captions.NOT_AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 10: FAILED to filter channel pending tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return
                
            writeToLog("INFO","Step 11: Going to verify channel pending tab entries filter by: " + enums.Captions.NOT_AVAILABLE.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED to verify channel pending tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return      
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in channel pending tab while using no search term")
