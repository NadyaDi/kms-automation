...
class Test:
    ...
    # Test Name : Sort by - Channels - when search is made
    # Test description:
    # create several channels and add them member / subscriber/ media
    # go to channels page, make a search and sort the channels :
...
#             if self.common.channel.verifyChannelsDefaultSort(enums.ChannelsSortBy.MEDIA_COUNT_NEWUI) == False:
            ...
            writeToLog("INFO","Step 4: Going to make a search in channels page")
            if self.common.channel.searchInMyChannelsAndChannelsWithoutVerifyResults(self.searchInMyChannels) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to make a search in channels page")
                return    
            ...
            writeToLog("INFO","Step 6: Going verify sort channels by 'Creation Date'")
            if self.common.channel.verifySortInMyChannels(enums.ChannelsSortBy.CREATION_DATE, self.sortByMostRecent) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED sort channels by 'Creation Date'")
                return 

            writeToLog("INFO","Step 7: Going verify sort channels by 'Media Count'")
#             if self.common.channel.verifySortInMyChannels(enums.ChannelsSortBy.MEDIA_COUNT_NEWUI,  self.sortByMediaCount) == False:
            if self.common.channel.verifySortInMyChannels(enums.ChannelsSortBy.MEDIA_COUNT,  self.sortByMediaCount) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED sort channels by 'Media Count'")
                return 
                  
            writeToLog("INFO","Step 8: Going verify sort channels by 'Alphabetical A-Z'")
            if self.common.channel.verifySortInMyChannels(enums.ChannelsSortBy.ALPHABETICAL_NEWUI, self.sortByAlphabetical) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED sort channels by 'Alphabetical A-Z'")
                return 
                
            writeToLog("INFO","Step 9: Going verify sort channels by 'Alphabetical Z-A'")
            if self.common.channel.verifySortInMyChannels(enums.ChannelsSortBy.ALPHABETICAL_Z_A_NEWUI, self.sortByAlphabeticalZToA) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED sort channels by 'Alphabetical Z-A'")
                return 
                 
            writeToLog("INFO","Step 10: Going verify sort channels by 'Members & Subscribers'")
            if self.common.channel.verifySortInMyChannels(enums.ChannelsSortBy.MEMBERS_AND_SUBSCRIBERS, self.sortByMembersAndSubscribers) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10 : FAILED sort channels by 'Members & Subscribers'")
                return 
            ##################################################################
            writeToLog("INFO","TEST PASSED: 'Channels - Sort Channels with search' was done successfully")
