...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Scheduling - no search - Add to Gallery - SR tab
    # Test description:
    # Verify that Add to Gallery - SR tab entries are properly displayed while being filtered by scheduling with no search term
    #================================================================================================================================
    testNum = "4501"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            if self.common.category.navigateToAddToCategory(self.gallery) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to navigate to Add to Gallery")
                return
            ...
            writeToLog("INFO", "STEP 3 Going to filter Add to Gallery SR Tab entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.ALL) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.ALL.value + "'")
                return

            writeToLog("INFO", "STEP 4 Going to verify filter Add to Gallery SR Tab entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.allAvailabilities, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 4: FAILED to verify filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.ALL.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 6 Going to filter Add to Gallery SR Tab entries by: " + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.FUTURE_SCHEDULING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
                return

            writeToLog("INFO", "STEP 7 Going to verify filter Add to Gallery SR Tab entries by: " + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.futureScheduling, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 7: FAILED to verify filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.FUTURE_SCHEDULING.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9 Going to filter Add to Gallery SR Tab entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.AVAILABLE_NOW) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return

            writeToLog("INFO", "STEP 10 Going to verify filter Add to Gallery SR Tab entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.availableNow, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 10: FAILED to verify filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 12 Going to filter Add to Gallery SR Tab entries by: " + enums.Scheduling.PAST_SCHEDULING.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.PAST_SCHEDULING) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return

            writeToLog("INFO", "STEP 13: Going to verify filter Add to Gallery SR Tab entries by: " + enums.Scheduling.PAST_SCHEDULING.value + "'")
            if self.common.channel.verifyFiltersInAddToChannel(self.pastScheduling, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 13: FAILED to verify filter Add to Gallery SR Tab entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Add to Gallery - SR tab while using filter by scheduling")
