...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name :Filter by Duration - with search - Channel page - media tab
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 1: Going to navigate to the Channel page - media tab")
            if self.common.channel.navigateToChannel(self.channelName, forceNavigate=True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to navigate to the Channel page - media tab")
                return

            writeToLog("INFO","Step 2: Going  to search by " + self.entryName + " in Channel page - media tab")
            if self.common.channel.searchInChannelWithoutVerifyResults(self.entryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 2: FAILED to search by " + self.entryName + " in Channel page - media tab")
                return

            writeToLog("INFO", "STEP 3: Going to filter Channel page - media tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return

            writeToLog("INFO", "STEP 4: Going to verify filter Channel page - media tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.allDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 4: FAILED to verify filter Channel page - media tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 6: Going to filter Channel page - media tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 7: Going to verify filter Channel page - media tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.tenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 7: FAILED to verify filter Channel page - media tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9: Going to filter Channel page - media tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 10: Going to verify filter Channel page - media tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 10: FAILED to verify filter Channel page - media tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 12: Going to filter Channel page - media tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return


            writeToLog("INFO", "STEP 13: Going to verify filter Channel page - media tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 13: FAILED to verify filter Channel page - media tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 15: Going to filter Channel page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 16: Going to verify filter Channel page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 16: FAILED to verify filter Channel page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 18: Going to filter Channel page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 19: Going to filter Channel page - media tab entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to filter Channel page - media tab entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")
                return

            writeToLog("INFO", "STEP 20: Going to verify filter Channel page - media tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.specialDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 20: FAILED to verify filter Channel page - media tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            if self.common.globalSearch.verifyFiltersInGlobalPage(self.betweenDuration) == False:
                ...
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Channel page - media tab while using filter by duration with a search term")
