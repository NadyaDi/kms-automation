...
class Test:
    ...
    # Test Name : eSearch - Search In - Add to channel - SR tab
    # Test description:
    # Upload entries with different cue points fields
    # Go Add to channel - SR tab, make a search, and filter entries by 'Search in' drop down:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 4: Going to verify that 'search in' dropdown is disabled before search")
            if self.common.myMedia.verifySearchInDropDownState(False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 4: FAILED to verify that 'search in' dropdown is disabled before search")
                return  
              
            writeToLog("INFO","Step 5: Going to make a search in 'Add to channel' - 'SR' tab")
            if self.common.channel.searchInAddToChannel(self.searchInAddToChannel, tabToSearcFrom=enums.AddToChannelTabs.SHARED_REPOSITORY) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 5: FAILED to make a search in 'Add to channel' - 'SR' tab")
                return          
             
            writeToLog("INFO","Step 6: Going to verify that 'search in' dropdown is enable after making a search")
            if self.common.myMedia.verifySearchInDropDownState(True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify that 'search in' dropdown is enable after making a search")
                return  
              
            writeToLog("INFO","Step 7: Going to select 'search in' option: " + enums.SearchInDropDown.QUIZ.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.QUIZ, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 7: FAILED to select 'search in' option: " + enums.SearchInDropDown.QUIZ.value)
                return              
            ...
            writeToLog("INFO","Step 9: Going to verify fields display for search in" + enums.SearchInDropDown.QUIZ.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForQuiz) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to displayed correct fields for " + enums.SearchInDropDown.QUIZ.value)
                return             
              
            writeToLog("INFO","Step 10: Going to select 'search in' option: " + enums.SearchInDropDown.ALL_FIELDS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.ALL_FIELDS, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10: FAILED to select 'search in' option: " + enums.SearchInDropDown.ALL_FIELDS.value)
                return              
            ...
            writeToLog("INFO","Step 12: Going to verify fields display for search in" + enums.SearchInDropDown.ALL_FIELDS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForAllFields) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to displayed correct fields for " + enums.SearchInDropDown.ALL_FIELDS.value)
                return               
              
            writeToLog("INFO","Step 13: Going to select 'search in' option: " + enums.SearchInDropDown.DETAILS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.DETAILS, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 13: FAILED to select 'search in' option: " + enums.SearchInDropDown.DETAILS.value)
                return              
            ...
            writeToLog("INFO","Step 15: Going to verify fields display for search in" + enums.SearchInDropDown.DETAILS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForDetails) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 15: FAILED to displayed correct fields for " + enums.SearchInDropDown.DETAILS.value)
                return                        
              
            writeToLog("INFO","Step 16: Going to select 'search in' option: " + enums.SearchInDropDown.CHAPTERS_AND_SLIDES.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.CHAPTERS_AND_SLIDES, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 16: FAILED to select 'search in' option: " + enums.SearchInDropDown.CHAPTERS_AND_SLIDES.value)
                return              
            ...
            writeToLog("INFO","Step 18: Going to verify fields display for search in" + enums.SearchInDropDown.CHAPTERS_AND_SLIDES.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForChaptersAndSlides) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 18: FAILED to displayed correct fields for " + enums.SearchInDropDown.CHAPTERS_AND_SLIDES.value)
                return                           
  
            writeToLog("INFO","Step 19: Going to select 'search in' option: " + enums.SearchInDropDown.POLLS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.POLLS, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 19: FAILED to select 'search in' option: " + enums.SearchInDropDown.POLLS.value)
                return              
              
            writeToLog("INFO","Step 20: Going to verify the correct entries are displayed in results")
            if self.common.channel.verifyFiltersInAddToChannel(self.searchInPolls, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 20: FAILED to displayed correct entries in results")
                return 
            
            writeToLog("INFO","Step 21: Going to verify fields display for search in" + enums.SearchInDropDown.POLLS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForPolls) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 21: FAILED to displayed correct fields for " + enums.SearchInDropDown.POLLS.value)
                return              
              
            writeToLog("INFO","Step 22: Going to select 'search in' option: " + enums.SearchInDropDown.CAPTIONS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.CAPTIONS, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 22: FAILED to select 'search in' option: " + enums.SearchInDropDown.CAPTIONS.value)
                return              
              
            writeToLog("INFO","Step 23: Going to verify the correct entries are displayed in results")
            if self.common.channel.verifyFiltersInAddToChannel(self.SearchIncaptions, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 23: FAILED to displayed correct entries in results")
                return       
            
            writeToLog("INFO","Step 24: Going to verify fields display for search in" + enums.SearchInDropDown.CAPTIONS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForCaptions) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 24: FAILED to displayed correct fields for " + enums.SearchInDropDown.CAPTIONS.value)
                return                     
            
            writeToLog("INFO","Step 25: Going to select 'search in' option: " + enums.SearchInDropDown.COMMENTS.value)
            if self.common.myMedia.selectSearchInDropDownOption(option=enums.SearchInDropDown.COMMENTS, location=enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 25: FAILED to select 'search in' option: " + enums.SearchInDropDown.COMMENTS.value)
                return              
            
            writeToLog("INFO","Step 26: Going to verify the correct entries are displayed in results")
            if self.common.channel.verifyFiltersInAddToChannel(self.searchInComments, enums.Location.ADD_TO_CHANNEL_SR) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 26: FAILED to displayed correct entries in results")
                return  
            
            writeToLog("INFO","Step 27: Going to verify fields display for search in" + enums.SearchInDropDown.COMMENTS.value +" in results")
            if self.common.myMedia.checkEntriesFieldsInResults(self.fieldsForComments) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 27: FAILED to displayed correct fields for " + enums.SearchInDropDown.COMMENTS.value)
                return                
            ...
            writeToLog("INFO","Step 29: Going to verify 'search in' dropdown is disabled after clearing search")
            if self.common.myMedia.verifySearchInDropDownState(False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 29: FAILED to dosabled 'search in' dropdown is disabled after clearing search")
                return   
            ##################################################################
            writeToLog("INFO","TEST PASSED: Search in Add to channel - SR tab done successfully")
