...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name :Filter by Duration - no search - Category page - pending tab
    # Test description:
    # Verify that category entries are properly displayed in pending tab while using no search term and filter them by Duration
    #================================================================================================================================
    testNum = "4468"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            if self.common.channel.navigateToPendingaTab(self.categoryName, location=enums.Location.CATEGORY_PAGE) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to navigate to Category page - pending tab")
                return

            writeToLog("INFO", "STEP 2: Going to filter Category page - pending tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter Category page - pending tab entries by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9: Going to verify filter Category page - pending tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter Category page - pending tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter global entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 15: Going to verify filter global entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to verify filter global entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 16: Going to verify filter Category page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 16: FAILED to verify filter Category page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 18: Going to filter Category page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Category page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 19: Going to filter Category page - pending tab entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to filter Category page - pending tab entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")
                return

            writeToLog("INFO", "STEP 20: Going to verify filter Category page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.specialDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 20: FAILED to verify filter Category page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.betweenDuration) == False:
                ...
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in category page pending tab while using filter by duration with no search term")
