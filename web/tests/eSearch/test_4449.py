...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 6: Going to close down the filters drop down menu")
            if self.common.base.click(self.common.category.CATEGORY_PENDING_TAB_FILTER_BUTTON, 20) == False:
                writeToLog("INFO","Step 6: FAILED to close the filters drop down menu")
                self.status = "Fail"
                return
            ...
            writeToLog("INFO","Step 9: Going to clear all the search filters")
            if self.common.base.click(self.common.category.CATEGORY_PENDING_TAB_FILTER_CLEAR_ALL_BUTTON, 20) == False:
                writeToLog("INFO","Step 9: FAILED to clear all the search filters")
                self.status = "Fail"
                return

            writeToLog("INFO", "Step 10: Going to wait for the filter changes")
            if self.common.general.waitForLoaderToDisappear() == False:
                writeToLog("INFO", "Step 10: FAILED to clear the filter changes")
                self.status = "Fail"
                return

            writeToLog("INFO","Step 11: Going to close down the filters drop down menu")
            if self.common.base.click(self.common.category.CATEGORY_PENDING_TAB_FILTER_BUTTON, 20) == False:
                writeToLog("INFO","Step 11: FAILED to close the filters drop down menu")
                self.status = "Fail"
                return

            writeToLog("INFO","Step 12: Going to filter category pending tab entries by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 12: FAILED to filter category pending tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 13: Going to verify category pending tab entries filter by: " + enums.Captions.NOT_AVAILABLE.value)
            if self.common.channel.verifyFiltersInPendingTab(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 13: FAILED to verify category pending tab entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
