...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - with search - My Media 
    # Test description:
    # Verify that the caption filters displays the right entries while using all / available and not available caption filters
    #================================================================================================================================
    testNum = "4051"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            if self.common.myMedia.searchEntryMyMedia(self.entryName, forceNavigate=True, exactSearch=False) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to navigate to " + self.entryName + " entry page")
                return
            
            writeToLog("INFO","Step 2: Going to filter my media entries by: " + enums.Captions.ALL.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 2: FAILED to filter my media entries  by '" + enums.Captions.ALL.value + "'")
                    return
                
            writeToLog("INFO","Step 3: Going to verify my media entries filter by: " + enums.Captions.ALL.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByAll) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to verify my media entries  by '" + enums.Captions.ALL.value + "'")
                return 
            ...
            writeToLog("INFO","Step 5: Going to filter my media entries by: " + enums.Captions.AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 5: FAILED to filter my media entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return
                 
            writeToLog("INFO","Step 6: Going to verify my media entries filter by: " + enums.Captions.AVAILABLE.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to verify my media entries  by '" + enums.Captions.AVAILABLE.value + "'")
                return
            ...
            writeToLog("INFO","Step 8: Going to close down the filters drop down menu") 
            if self.common.base.click(self.common.myMedia.MY_MEDIA_FILTERS_BUTTON_NEW_UI, 20) == False:
                writeToLog("INFO","Step 8: FAILED to close the filters drop down menu")
                self.status = "Fail"
                return
            ...
            writeToLog("INFO","Step 10: Going to filter my media entries by: " + enums.Captions.NOT_AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 10: FAILED to filter my media entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return
                
            writeToLog("INFO","Step 11: Going to verify my media entries filter by: " + enums.Captions.NOT_AVAILABLE.value)  
            if self.common.myMedia.verifyFiltersInMyMedia(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED to verify my media entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed while using caption filters")
