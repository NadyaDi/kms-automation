...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name :Filter by Duration - no search - Channel page - pending tab
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 2: Going to filter Channel page - pending tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return

            writeToLog("INFO", "STEP 3: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.allDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 5: Going to filter Channel page - pending tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 5: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 6: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.tenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 8: Going to filter Channel page - pending tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 8: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return

            writeToLog("INFO", "STEP 9: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 11: Going to filter Channel page - pending tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 11: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return


            writeToLog("INFO", "STEP 12: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 14: Going to filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 14: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 15: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 17: Going to filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 17: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return

            writeToLog("INFO", "STEP 18: Going to filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")
                return

            writeToLog("INFO", "STEP 19: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.specialDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.betweenDuration) == False:
                ...
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Channel page - pending tab while using filter by duration with no search term")
