...
class Test:
    ...
    #================================================================================================================================
    #  @Author: oded berihon
    # Test Name : eSearch - Filter by media type in category pending tab page with search
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.channelForEsearch  = "category for eSearch moderator"
            ...
            if self.common.channel.navigateToPendingaTab(self.channelForEsearch, location=enums.Location.CATEGORY_PAGE) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 2: FAILED to go to add to category page")
                return
            
            writeToLog("INFO","Step 3: Going to search entry in pending tab category page")
            if self.common.channel.makeSearchInPending(self.entryName1) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 3: FAILED to earch entry in pending tab category page")
                return
...
#              tmpType = (self.common.myMedia.MY_MEDIA_DROPDOWNLIST_ITEM_NEW_UI[0], self.common.myMedia.MY_MEDIA_DROPDOWNLIST_ITEM_NEW_UI[1].replace("DROPDOWNLIST_ITEM", enums.MediaType.ALL_MEDIA.value))                
            ...
            writeToLog("INFO","Step 6: Going to search entry in pending tab category page")
            if self.common.channel.makeSearchInPending(self.entryName2) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to earch entry in pending tab category page")
                return
                
            sleep(1)
            writeToLog("INFO","Step 7: Going to filter entries in add to category page by: " + enums.MediaType.AUDIO.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.AUDIO) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 7: FAILED to filter entries in add to category page by '" + enums.MediaType.AUDIO.value + "'")
                    return
                    
            writeToLog("INFO","Step 8: Going to verify entries in add to category page filter by: " + enums.MediaType.AUDIO.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByAudio) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 8: FAILED to verify entries in add to category page filter by '" + enums.MediaType.AUDIO.value + "'")
                return
            ...
            writeToLog("INFO","Step 9: Going to search entry in pending tab category page")
            if self.common.channel.makeSearchInPending(self.entryName3) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to earch entry in pending tab category page")
                return
                   
            sleep(1)
            writeToLog("INFO","Step 10: Going to filter entries in add to category page by: " + enums.MediaType.VIDEO.value) 
            if self.common.isElasticSearchOnPage() == True:
            ...
            writeToLog("INFO","Step 11: Going to verify entries in add to category page filter by: " + enums.MediaType.VIDEO.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByVideo) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED to verify entries in add to category page filter by '" + enums.MediaType.VIDEO.value + "'")
                return 
            ...
            writeToLog("INFO","Step 12: Going to search entry in pending tab category page")
            if self.common.channel.makeSearchInPending(self.entryNameQuiz) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to earch entry in pending tab category page")
                return
                
            sleep(1)
            writeToLog("INFO","Step 13: Going to filter entries in add to category page by: " + enums.MediaType.QUIZ.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.MEDIA_TYPE, enums.MediaType.QUIZ) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 13: FAILED to filter entries in add to category page by '" + enums.MediaType.QUIZ.value + "'")
                    return
                 
            writeToLog("INFO","Step 14: Going to verify entries in add to category page filter by: " + enums.MediaType.QUIZ.value)  
            if self.common.channel.verifyFiltersInPendingTab(self.filterByQuiz) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 14: FAILED to verify entries in add to category page filter by '" + enums.MediaType.QUIZ.value + "'")
                return 
            ...
            ##################################################################
            writeToLog("INFO","TEST PASSED: Sort by media type in category pending tab page done successfully")
