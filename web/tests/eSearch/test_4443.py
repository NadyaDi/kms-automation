...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - no search - Channel page - media tab    
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 8: Going to filter channel entries by: " + enums.Captions.NOT_AVAILABLE.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.NOT_AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 8: FAILED to filter channel entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                    return
                 
            writeToLog("INFO","Step 9: Going to verify channel entries filter by: " + enums.Captions.NOT_AVAILABLE.value)  
            if self.common.channel.verifyEntriesDisplay(self.filterByNotAvailable) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to verify channel entries  by '" + enums.Captions.NOT_AVAILABLE.value + "'")
                return               
            ##################################################################
