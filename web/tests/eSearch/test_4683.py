...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name :Filter by Duration - with search - Channel page - pending tab
    # Test description:
    # Verify that channel entries are properly displayed in media tab while using a search term and filter them by Duration
    #================================================================================================================================
    testNum = "4683"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 1: Going to navigate to the Channel page - pending tab")
            if self.common.channel.navigateToPendingaTab(self.channelName, enums.Location.CHANNEL_PAGE) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 1: FAILED to navigate to the Channel page - pending tab")
                return

            writeToLog("INFO","Step 2: Going  to search by " + self.entryName + " in Channel page - pending tab")
            if self.common.channel.makeSearchInPending(self.entryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 2: FAILED to search by " + self.entryName + " in Channel page - pending tab")
                return
 
            writeToLog("INFO", "STEP 3: Going to filter Channel page - pending tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.ANY_DURATION) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 3: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
 
            writeToLog("INFO", "STEP 4: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.ANY_DURATION.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.allDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 4: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.ANY_DURATION.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 6: Going to filter Channel page - pending tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.TEN_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 6: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
 
            writeToLog("INFO", "STEP 7: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.TEN_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.tenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 7: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.TEN_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9: Going to filter Channel page - pending tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.THIRTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
 
            writeToLog("INFO", "STEP 10: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.THIRTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.thirtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 10: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.THIRTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 12: Going to filter Channel page - pending tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.SIXTY_MINUTES) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
 
 
            writeToLog("INFO", "STEP 13: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.SIXTY_MINUTES.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.sixtyDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 13: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.SIXTY_MINUTES.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 15: Going to filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 15: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
 
            writeToLog("INFO", "STEP 16: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.customDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 16: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 18: Going to filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.DURATION, enums.Duration.CUSTOM) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 18: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
 
            writeToLog("INFO", "STEP 19: Going to filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + " using special entry limits")
            if self.common.myMedia.filterCustomDuration(55)== False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 19: FAILED to filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + " using special entry limits")
                return
 
            writeToLog("INFO", "STEP 20: Going to verify filter Channel page - pending tab entries by: " + enums.Duration.CUSTOM.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.specialDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 20: FAILED to verify filter Channel page - pending tab entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.betweenDuration) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 26: FAILED to verify filter Global page entries  by '" + enums.Duration.CUSTOM.value + "'")
                return
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the entries are properly displayed in Channel page - pending tab while using filter by duration with a search term")
