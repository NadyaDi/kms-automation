...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Scheduling - with no - Category page - pending tab
    # Test description:
    # Verify that Category page - pending tab entries are properly displayed while being filtered by scheduling with no search term
    #================================================================================================================================
    testNum = "4491"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO", "STEP 2: Going to filter Category page - pending tab entries by: " + enums.Scheduling.ALL.value + "'")
            if self.common.myMedia.SortAndFilter(enums.SortAndFilter.SCHEDULING, enums.Scheduling.ALL) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 2: FAILED to filter Category page - pending tab entries by '" + enums.Scheduling.ALL.value + "'")
                return
            ...
            writeToLog("INFO", "STEP 9: Going to verify filter Category page - pending tab entries by: " + enums.Scheduling.AVAILABLE_NOW.value + "'")
            if self.common.channel.verifyFiltersInPendingTab(self.availableNow) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 9: FAILED to verify filter Category page - pending tab entries  by '" + enums.Scheduling.AVAILABLE_NOW.value + "'")
                return
            ...
            if self.common.channel.verifyFiltersInPendingTab(self.pastScheduling) == False:
                self.status = "Fail"
                writeToLog("INFO", "STEP 12: FAILED to verify filter global entries  by '" + enums.Scheduling.PAST_SCHEDULING.value + "'")
                return
            ##################################################################
