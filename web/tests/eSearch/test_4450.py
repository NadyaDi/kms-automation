...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Filter by Captions - no search - Category page - pending tab
    # Test description:
    # Verify that category pending tab entries are properly displayed while using no search term and filter them by captions
    #================================================================================================================================
    testNum = "4450"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 2: Going to filter category pending tab entries by: " + enums.Captions.ALL.value)
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 2: FAILED to filter category pending tab entries  by '" + enums.Captions.ALL.value + "'")
                    return

            writeToLog("INFO","Step 3: Going to verify category pending tab entries filter by: " + enums.Captions.ALL.value)
            ...
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 5: FAILED to filter category pending tab entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return

            writeToLog("INFO","Step 6: Going to verify category pending tab entries filter by: " + enums.Captions.AVAILABLE.value)
