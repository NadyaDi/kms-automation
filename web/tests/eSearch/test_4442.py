...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 2: Going to navigate to search by " + self.entryName + " in channel")
            if self.common.channel.searchInChannelWithoutVerifyResults(self.entryName) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 2: FAILED to search by " + self.entryName + " in channel")
                return
            
            writeToLog("INFO","Step 3: Going to filter channel entries by: " + enums.Captions.ALL.value) 
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.ALL) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 3: FAILED to filter channel entries  by '" + enums.Captions.ALL.value + "'")
                    return
                
            writeToLog("INFO","Step 4: Going to verify channel entries filter by: " + enums.Captions.ALL.value)  
            ...
            if self.common.isElasticSearchOnPage() == True:
                if self.common.myMedia.SortAndFilter(enums.SortAndFilter.CAPTIONS, enums.Captions.AVAILABLE) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step 6: FAILED to filter channel entries  by '" + enums.Captions.AVAILABLE.value + "'")
                    return
                
            writeToLog("INFO","Step 7: Going to verify channel entries filter by: " + enums.Captions.AVAILABLE.value)  
