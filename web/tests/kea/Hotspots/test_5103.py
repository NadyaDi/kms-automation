...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Hotspots: Duplicate Hotspots
    # Test description:
    # Create a new entry with Five hotspots, and verify them in both time line section and entry page
    # Duplicate a hotspot, leaving six hotspots and verifying them in both time line section and entry page
    #================================================================================================================================
    testNum = "5103"
    ...
    typeTest            = "For a new Entry that was verified while having five hotspots and then six by duplicating one, in both timeline section and entry page"
    ...
    hotspotOneDuplicate   = ['Hotspot Title One Duplicated', enums.keaLocation.TOP_RIGHT, 0, 10, 'https://autoone.kaltura.com/', enums.textStyle.BOLD, '', '', 18, 12]
    ...
    # This Dictionary is used in order to verify the hotspots after one hotspot has been duplicated in player
    hotspotsDictUpdatedPlayer    = {'1':hotspotOneDuplicate, '2':hotspotOne, '3':hotspotTwo, '4':hotspotThree, '5':hotspotFour, '6':hotspotFive}
    
    # This Dictionary is used in order to verify the hotspots after one hotspot has been duplicated in player
    hotspotsDictUpdatedTimeline  = {'1':hotspotOne, '2':hotspotOneDuplicate, '3':hotspotTwo, '4':hotspotThree, '5':hotspotFour, '6':hotspotFive}
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.entryName             = clsTestService.addGuidToString("Hotspots - With one Hotspot Duplicated", self.testNum)
            ...
            writeToLog("INFO","Step 4: Going to verify the timeline section for " + self.entryName +" entry, before duplicating the " + self.hotspotOne[0])
            if self.common.kea.hotspotTimelineVerification(self.hotspotsDict, 5) == False:
                writeToLog("INFO","Step 4: FAILED to verify the timeline section for " + self.entryName +" entry, before duplicating the " + self.hotspotOne[0])
                return
            ...
            writeToLog("INFO","Step 8: Going to Duplicate the " + self.hotspotOne[0] + " hotspot from " + self.entryName)
            if self.common.kea.hotspotActions(self.hotspotOne[0], enums.keaHotspotActions.DUPLICATE) == False:
                writeToLog("INFO","Step 8: FAILED to Duplicate the " + self.hotspotOne[0] + " hotspot from " + self.entryName)
                return
                
            writeToLog("INFO","Step 9: Going to verify the timeline section for " + self.entryName +" entry, after duplicating the " + self.hotspotOne[0])
            if self.common.kea.hotspotTimelineVerification(self.hotspotsDictUpdatedTimeline, 6) == False:
                writeToLog("INFO","Step 9: FAILED to verify the timeline section for " + self.entryName +" entry, after duplicating the " + self.hotspotOne[0])
                return
            ...
            if self.common.player.hotspotVerification(self.hotspotsDictUpdatedTimeline, presentedHotspotsDetailsList) == False:
