...
class Test:
    ...
    # Test Name : Quiz - Analytics - Remove Last attempts - Average score type
    # Test description:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.entryName = clsTestService.addGuidToString("Quiz - Remove last attempt - Average score", self.testNum)
            self.quizEntryName = clsTestService.addGuidToString("Quiz - Remove last attempt - Average score - Quiz", self.testNum)      
            ...
            if self.common.player.verifyQuizAttempts(self.quizQuestionsDict2FirstAttempts, expectedQuizScore=self.quizSpecificAttemptScore, totalGivenAttempts=self.totalGivenAttempts, expectedAttemptGeneralScore=self.quizGeneral2FirstAttemptsScore, scoreType=enums.playerQuizScoreType.AVERAGE) == False:
                self.status = "Fail"
            ...
            if self.common.quizAnalytics.verifyUserAttemptsAndScore(self.loginUsername, self.userName, self.numberOfAttemptsBeforeDeletion, self.scoreBeforeDeletion, enums.playerQuizScoreType.AVERAGE, self.quizEntryName, True) == False:
                self.status = "Fail"
            ...
            if self.common.quizAnalytics.verifyUserAttemptsAndScore(self.loginUsername , self.userName + "   Last Attempt removed", self.numberOfAttemptsAfterDeletion, self.scoreAfterDeletion, enums.playerQuizScoreType.AVERAGE) == False:
                self.status = "Fail"
            ...
            if self.common.quizAnalytics.verifyUserAttemptsAndScore(self.loginUsername, self.userName, self.numberOfAttemptsAfterRetaking, self.scoreAfterRetaking, enums.playerQuizScoreType.AVERAGE, self.quizEntryName, True) == False:
