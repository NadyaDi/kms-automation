...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 6 : Going to verify that all the available quiz questions from the " + self.newEntryName + " entry remained answered")  
            if   self.common.player.quizVerification(self.questionDict, self.expectedQuizStateSubmitted, submittedQuiz=True, resumeQuiz=False, newQuiz=False, expectedQuizScore=str(100), location=enums.Location.ENTRY_PAGE, timeOut=60) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6 : FAILED to Going to verify that all the available quiz questions from the " + self.newEntryName + " entry remained answered")
                return                  
            ##################################################################
            self.status = "Pass"
            writeToLog("INFO","TEST PASSED: Open-Q is with hint and why added sucssefully")
