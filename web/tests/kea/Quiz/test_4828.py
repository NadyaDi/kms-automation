...
class Test:
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            ##################################################################
            self.entryName                  = clsTestService.addGuidToString("Quiz - Secure Embed ON - Download functionality Anonymous", self.testNum)
            self.entryNameQuiz              = clsTestService.addGuidToString("Quiz - Secure Embed ON - Download functionality Anonymous - Quiz", self.testNum)
            ...
            writeToLog("INFO","Step 7: Going to authenticate using " + self.userName + " account, in order to verify secure embed ON")
            if self.common.login.loginToKMSEmbed(self.userName, self.password) == False:
                writeToLog("INFO", "Step 7:FAILED to authenticate using " + self.userName + " account, in order to verify secure embed ON")
                return
