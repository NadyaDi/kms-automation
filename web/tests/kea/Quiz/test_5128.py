...
class Test:
    ...
    # Go to quiz page with different user
    #================================================================================================================================
    testNum = "5128"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.entryName = clsTestService.addGuidToString("Quiz - Remove last attempt", self.testNum)
            self.quizEntryName = clsTestService.addGuidToString("Quiz - Remove last attempt - Quiz", self.testNum)      
            ...
            writeToLog("INFO","Step 6: Going to navigate to KEA Quiz tab for " + self.quizEntryName)  
            if self.common.kea.initiateQuizFlow(self.quizEntryName, True, 1) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 6: FAILED to navigate to KEA Quiz tab for " + self.quizEntryName)  
                return 
            ...
            if self.common.player.verifyQuizAttempts(self.quizQuestionsDict2FirstAttempts, expectedQuizScore=self.quizSpecificAttemptScore, totalGivenAttempts=self.totalGivenAttempts, expectedAttemptGeneralScore=self.quizGeneralAttemptsScore, scoreType=enums.playerQuizScoreType.LATEST) == False:
                self.status = "Fail"
            ...
            if self.common.quizAnalytics.verifyUserAttemptsAndScore(self.loginUsername, self.userName, self.numberOfAttemptsBeforeDeletion, self.scoreBeforeDeletion, enums.playerQuizScoreType.LATEST) == False:
                self.status = "Fail"
            ...
            if self.common.quizAnalytics.verifyUserAttemptsAndScore(self.loginUsername, self.userName, self.numberOfAttemptsAfterDeletion, self.scoreAfterDeletion, enums.playerQuizScoreType.LATEST) == False:
                self.status = "Fail"
