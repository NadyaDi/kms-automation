...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Quiz Scores section in Entry Page - Disabled
    # Test description:
    # Verify that all the KEA Quiz Scores options have proper functionality by:
    # Verifying that the "Do not Show Scores", "Show Scores" and "Include Answers" can be disabled
    #================================================================================================================================
    testNum = "4768"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            ##################################################################
            self.entryName          = clsTestService.addGuidToString("Quiz - Score Disabled", self.testNum)
            self.newEntryName       = clsTestService.addGuidToString("Quiz - Score Disabled - Quiz", self.testNum)            
            # this dictionary is used in order to edit the options      
            self.doNotShowShowScoreDisabled         = {enums.KEAQuizOptions.DO_NOT_SHOW_SCORES:False}
            ...
            self.doNotShowShowScoreDisabledList     = [enums.KEAQuizOptions.DO_NOT_SHOW_SCORES, self.showScoreDisabledWithIncludeAnswerDisabled, self.doNotShowShowScoreDisabled]
            self.showScoresDisabledList             = [enums.KEAQuizOptions.SHOW_SCORES, self.showScoreEnabledWithIncludedAnswersEnabled, self.showScoresDisabled]
            self.includeAnswersDisabledList         = [enums.KEAQuizOptions.INCLUDE_ANSWERS, self.includeAnswersDict, self.includeAnswersDisabled]

            self.scoreOptionDisabledList            = [self.doNotShowShowScoreDisabledList, self.showScoresDisabledList, self.includeAnswersDisabledList]
            ...
            for option in self.scoreOptionDisabledList:
                ...
                writeToLog("INFO","Step " + str(i) + ": Going to change the " + enums.KEAQuizSection.SCORES.value + " option by disabling the " + option[0].value)  
                if self.common.kea.editQuizOptions(enums.KEAQuizSection.SCORES, option[2]) == False:
                    self.status = "Fail"
                    writeToLog("INFO","Step " + str(i) + ": FAILED to change the " + enums.KEAQuizSection.SCORES.value + " option by disabling the " + option[0].value)   
                    return
                else:
                    i = i + 1
                ...
                if self.common.login.loginToKMS(self.userName, self.password) == False:
                    ...
                    i = i + 1
            ##################################################################
            writeToLog("INFO","TEST PASSED: All the KEA Score options were properly disabled and verified in KEA Entry Page")
