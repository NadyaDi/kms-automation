...
class Test:
    ...
    # Test Name : Clipping a Quiz entry
    # Test description:
    # 
    #================================================================================================================================
    testNum = "4513"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            writeToLog("INFO","Step 5: Going to clip the entry from 12sec to 22sec - (2 quiz questions will be removed)")  
            if self.common.kea.clipEntry(self.videoEntryName + " - Quiz", "00:12", "00:22", expectedEntryDuration, enums.Location.EDIT_ENTRY_PAGE, enums.Location.MY_MEDIA, True) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 5: FAILED to clip the entry from 12sec to 22sec - (2 quiz questions will be removed)")
                return
 
            writeToLog("INFO","Step 6: Going to collect the new Quiz's question information from the player (after the Quiz was trimmed)")
            self.qestionCollectedafterTrim = self.common.player.collectQuizQuestionsFromPlayer("Clip of " + self.videoEntryName + " - Quiz", 3)
    ...
    def teardown_method(self,method):
        try:
            ...
            self.common.myMedia.deleteEntriesFromMyMedia([self.videoEntryName, self.videoEntryName + " - Quiz", "Clip of " + self.videoEntryName + " - Quiz"])
