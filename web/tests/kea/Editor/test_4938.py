...
class Test:
    ...
    #================================================================================================================================
    #  @Author: Horia Cus
    # Test Name : Editor: Editor: Redo timeline functionality
    # Test description:
    # Verify that the Redo option has functionality after performing the following actions to a section:
    # 1. Delete
    # 2. Split
    # 3. Set In
    # Undo each previously action in order to be able to redo
    #================================================================================================================================
    testNum     = "4938"
    ...
    testType            = "Entry that had sections split,deleted and set in, then undo the actions and then redo them"
    ...
    def test_01(self,driverFix,env):
        ...
        try:
            ...
            self.entryName = clsTestService.addGuidToString("Editor Redo functionality", self.testNum)
            ...
            writeToLog("INFO","Step 9: Going to use the Redo option in order re-delete a section")  
            if self.common.kea.timeLineUndoRedoReset(enums.KeaEditorTimelineOptions.REDO) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 9: FAILED to use the Redo option in order re-delete a section")  
                return
            
            writeToLog("INFO","Step 10: Going to use the Redo option in order re-split a section")  
            if self.common.kea.timeLineUndoRedoReset(enums.KeaEditorTimelineOptions.REDO) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 10: FAILED to use the Redo option in order re-split a section")  
                return 
            
            writeToLog("INFO","Step 11: Going to use the Redo option in order re-set in a section")  
            if self.common.kea.timeLineUndoRedoReset(enums.KeaEditorTimelineOptions.REDO) == False:
                self.status = "Fail"
                writeToLog("INFO","Step 11: FAILED to use the Redo option in order re-set in a section")  
                return
            
            writeToLog("INFO","Step 12: Going to save the KEA Editor Timeline section for " + self.entryName + " entry")
            if self.common.kea.saveEditorChanges(True)== False:
                self.status = "Fail"
                writeToLog("INFO","Step 12: FAILED to save the KEA Editor Timeline section for " + self.entryName + " entry")
                return
            
            writeToLog("INFO","Step 13: Going to compare the time length of the entry from KEA Editor and Entry Page for " + self.entryNameClipped + " entry")
            if self.common.kea.compareEntryDurationInKeaAndEntryPage(self.entryNameClipped, '00:15')== False:
                self.status = "Fail"
                writeToLog("INFO","Step 13: FAILED to compare the time length of the entry from KEA Editor and Entry Page for " + self.entryNameClipped + " entry")
                return          
            #########################################################################
